...
import {
  Calendar, Plus, Edit3, Trash2, Users, Star, CheckCircle, BellRing, Download, Eye,
  Clock, MapPin, UserCheck, Trophy, BarChart3, Settings, LogOut, Filter, Search,
  TrendingUp, Award, Menu, UserCircle2, UserPen, X, ChevronLeft, ChevronRight,
  ScanLine, ImageIcon, Upload, RefreshCw, Bot, Crown, ChevronDown
} from "lucide-react";
...
export default function HostDashboard() {
  ...
  const handleScanSuccess = async (decodedText) => {
    try {
      ...
      await api.put(`/api/host/events/${data.eventId}/registrations/${data.studentId}/attendance`, { attended: true }, { headers: bearer() });
  ...
  useEffect(() => {
    ...
    const handleEventSelect = async (event) => {
      ...
      try {
        ...
        const pStats = api.get(`/api/reviews/events/${event._id}/reviews/stats`, { headers: bearer() });
    ...
    const fetchAiInsights = async (eventId) => {
      ...
      try {
        ...
        const res = await api.get(`/api/reviews/events/${eventId}/reviews/ai-insights`, { headers: bearer() });
        if (res.status === 204) {
    ...
    const classifyEvent = async (eventId) => {
      try {
        ...
        const { data } = await api.post(`/api/host/events/${eventId}/classify`, {}, { headers: bearer() });
            ...
            if (response.ok) {
              ...
              if (data && data.address) {
                ...
                newLocationData = {
                  ...
                  state: state || "",
                  pincode: postcode || "",
                  location: data.name || road || suburb || city || town || village || "Current Location"
                };
    ...
    const viewNotification = async (n) => {
      if (!n.read) {
        ...
        setNotifications(prev => prev.map(notif => notif._key === n._key ? { ...notif, read: true } : notif));
      }
    ...
    const submitForm = async (e) => {
      ...
      try {
        const payload = {
          ...
          coordinates: (form.latitude && form.longitude) ? [parseFloat(form.longitude), parseFloat(form.latitude)] : undefined
        };
    ...
    const loadFeedbacks = async (event) => {
      ...
      try {
        ...
        try {
          const statsRes = await api.get(`/api/reviews/events/${event._id}/stats`, { headers: bearer() });
          ...
          >
            <div className="px-6 h-full flex items-center justify-between max-w-[1920px] mx-auto">
              <div className="flex items-center space-x-6">
                ...
                <div className="flex items-center gap-2 group cursor-pointer" onClick={() => navigate('/host/dashboard')}>
                    ...
                    <div className="absolute right-0 mt-4 w-80 bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] z-50">
                      ...
                      <div className="max-h-80 overflow-y-auto">
                        {(notifications || []).length === 0 ? (
                          <div className="p-4 text-sm text-slate-500 italic text-center">No new alerts</div>
                        ) : (
                          (notifications || []).map((n, idx) => (
                            <div
                              key={n._key || idx}
                              onClick={() => { setNotifOpen(false); viewNotification(n); }}
                            ...
                            >
                              <div className={`mt-1 w-2 h-2 rounded-full flex-shrink-0 ${!n.read ? 'bg-blue-600' : 'bg-gray-300'}`} />
                              <div className="flex-1">
                                <p className="text-sm font-bold text-black leading-tight mb-1">{n.title || "Alert"}</p>
                                <p className="text-sm font-medium text-black leading-tight line-clamp-2">{n.message}</p>
                                <p className="text-xs text-slate-500 mt-1 font-mono">{n.at ? new Date(n.at).toLocaleTimeString() : ''}</p>
                              </div>
                            </div>
                          ))
                        )}
                      ...
                      <div className="p-4 border-b-2 border-black bg-neutral-50">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="font-bold text-black truncate uppercase tracking-tight">{user?.fullname || "Host"}</div>
                  ...
                  {[
                    ...
                    >
                      ...
                      <span className="relative z-10 flex items-center gap-4">
                        <Icon className={`w-5 h-5 ${activeTab === id ? 'text-white' : 'text-slate-400 group-hover:text-black'}`} />
              ...
              <div className="max-w-7xl mx-auto">
                ...
                {activeTab === "events" && (
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    {[
                      { label: "Total Events", value: stats.total, icon: Calendar, bg: "bg-blue-100" },
                      { label: "Completed", value: stats.completed, icon: CheckCircle, bg: "bg-green-100" },
                      { label: "Upcoming", value: stats.upcoming, icon: Clock, bg: "bg-yellow-100" },
                      { label: "Registrations", value: stats.totalRegistrations, icon: UserCheck, bg: "bg-purple-100" },
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-black text-xs font-black uppercase tracking-widest mb-2">{label}</p>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {profileJustSavedAt && (Date.now() - profileJustSavedAt < 3500) && (
                          <span className="text-sm font-bold border-2 border-green-600 bg-green-100 text-green-800 px-2 py-1 uppercase">Saved!</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="bg-white p-6 border-2 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                      ...
                      <div className="relative group">
                        ...
                        <div className="relative border-2 border-black bg-neutral-100 h-48 overflow-hidden group-hover:bg-neutral-200 transition-colors">
                          {hostProfile.bannerUrl ? (
                            <img src={hostProfile.bannerUrl} alt="Banner" className="w-full h-full object-cover transition-all duration-500 hover:grayscale" />
                          ) : (
                            <div className="w-full h-full flex items-center justify-center text-neutral-400 font-bold uppercase tracking-widest">No Banner Uploaded</div>
                          )}
                      ...
                      <div className="mt-8">
                        ...
                        <div className="flex items-center gap-6">
                          <div className="w-24 h-24 rounded-none border-2 border-black bg-neutral-100 overflow-hidden shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                            <img src={hostProfile.profilePic || ''} alt="Profile" className="w-full h-full object-cover transition-all duration-500 hover:grayscale" onError={(e) => { e.currentTarget.style.visibility = 'hidden'; }} />
                          </div>
                      ...
                      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="md:col-span-2">
                          <label className="block text-xs font-bold uppercase text-black mb-2">Bio</label>
                          <textarea
                            rows={4}
                        ...
                        <div>
                          <label className="block text-xs font-bold uppercase text-black mb-2">Website</label>
                          <input
                            type="url"
                          ...
                          {hostTouched.website && hostProfileErrors.website && (
                            <p className="text-xs font-bold text-red-600 mt-1 uppercase">{hostProfileErrors.website}</p>
                          )}
                        </div>
                        <div>
                          <label className="block text-xs font-bold uppercase text-black mb-2">Twitter / X</label>
                          <input
                            type="text"
                          ...
                          {hostTouched.twitter && hostProfileErrors.twitter && (
                            <p className="text-xs font-bold text-red-600 mt-1 uppercase">{hostProfileErrors.twitter}</p>
                          )}
                        </div>
                        <div>
                          <label className="block text-xs font-bold uppercase text-black mb-2">Instagram</label>
                          <input
                            type="text"
                          ...
                          {hostTouched.instagram && hostProfileErrors.instagram && (
                            <p className="text-xs font-bold text-red-600 mt-1 uppercase">{hostProfileErrors.instagram}</p>
                          )}
                        </div>
                        <div>
                          <label className="block text-xs font-bold uppercase text-black mb-2">LinkedIn</label>
                          <input
                            type="text"
                          ...
                          {hostTouched.linkedin && hostProfileErrors.linkedin && (
                            <p className="text-xs font-bold text-red-600 mt-1 uppercase">{hostProfileErrors.linkedin}</p>
                          )}
                        </div>
                      </div>
                    </div>
                  ...
                  <>
                    ...
                    {loading ? (
                      ...
                      <div className="text-center py-20 bg-white border-2 border-dashed border-black">
                        ...
                        <h3 className="text-2xl font-black uppercase text-black mb-2">No Events Found</h3>
                            ...
                            <div className="relative h-48 bg-neutral-100 border-b-2 border-black overflow-hidden">
                              {event.imageUrl ? (
                                ...
                                <div className="w-full h-full flex items-center justify-center bg-neutral-100 relative overflow-hidden">
                                  <div className="absolute inset-0 opacity-10" style={{ backgroundImage: 'radial-gradient(circle at 1px 1px, black 1px, transparent 0)', backgroundSize: '10px 10px' }}></div>
                            ...
                            <div className="p-6 flex-1 flex flex-col">
                              ...
                              <div className="space-y-3 mb-6 font-mono text-xs font-medium">
                                <div className="flex items-center gap-3 text-slate-700">
                                  ...
                                  <span className="uppercase">{new Date(event.date).toLocaleDateString()}</span>
                                </div>
                                <div className="flex items-center gap-3 text-slate-700">
                                  <MapPin className="w-4 h-4 text-black" />
                                  <span className="line-clamp-1 uppercase">{event.location || "TBA"}</span>
                                </div>
                                <div className="flex items-center gap-3 text-slate-700">
                                ...
                                {event.isOnline && (
                                  <div className="flex items-center gap-3 text-blue-700 font-bold uppercase">
                                    <span className="flex items-center gap-2"><div className="w-2 h-2 bg-blue-600 rounded-full animate-pulse"></div> Online Event</span>
                                  </div>
                                )}
                                {event.price > 0 && (
                              ...
                              {event.tags && event.tags.length > 0 && (
                                <div className="flex flex-wrap gap-2 mb-6">
                                  {event.tags.slice(0, 3).map((tag, idx) => (
                                    <span key={idx} className="px-2 py-0.5 border-2 border-black bg-neutral-100 text-black text-xs font-bold uppercase">
                                      {tag}
                                    </span>
                                  ))}
                                  {event.tags.length > 3 && (
                              ...
                              {Array.isArray(event.images) && event.images.length > 0 && (
                                <div className="mt-4 pt-4 border-t-2 border-dashed border-black/20">
                                  <div className="text-xs font-bold uppercase text-black mb-2">Gallery ({event.images.length})</div>
                    ...
                    {selectedEvent ? (
                      <div className="bg-white border-2 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                        ...
                        <div className="flex items-center justify-between gap-3 mb-6">
                          <div className="flex items-center gap-2">
                            <input value={regSearch} onChange={e => setRegSearch(e.target.value)} placeholder="Search name or email" className="px-3 py-2 bg-white border-2 border-black text-sm outline-none font-medium placeholder-slate-400 focus:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all" />
                            <select value={regStatus} onChange={e => setRegStatus(e.target.value)} className="px-3 py-2 bg-white border-2 border-black text-sm outline-none font-bold uppercase focus:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all">
                          ...
                          <div className="flex items-center gap-3">
                            <button onClick={downloadServerXlsx} className="px-3 py-2 text-xs font-bold uppercase bg-white border-2 border-black hover:bg-neutral-100 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[-1px] hover:translate-y-[-1px] transition-all">Download XLSX</button>
                            ...
                            <table className="w-full border-collapse">
                              <thead>
                                <tr className="border-b-2 border-black bg-neutral-100">
                                  <th className="text-left py-3 px-4 text-black text-xs font-black uppercase tracking-wider">Student Name</th>
                                  <th className="text-left py-3 px-4 text-black text-xs font-black uppercase tracking-wider">Email</th>
                                  <th className="text-left py-3 px-4 text-black text-xs font-black uppercase tracking-wider">Registered At</th>
                                  <th className="text-left py-3 px-4 text-black text-xs font-black uppercase tracking-wider">Status</th>
                                  <th className="text-left py-3 px-4 text-black text-xs font-black uppercase tracking-wider">Attended</th>
                                </tr>
                              </thead>
                              <tbody>
                                {filteredRegistrations.map((reg, idx) => (
                                  <tr key={idx} className="border-b border-black/10 hover:bg-neutral-50 transition-colors">
                                    <td className="py-3 px-4 font-bold text-black">{reg.studentId?.fullname || "-"}</td>
                                    <td className="py-3 px-4 text-slate-600 font-mono text-xs">{reg.studentId?.email || "-"}</td>
                                    <td className="py-3 px-4 text-slate-600 font-mono text-xs">
                                      {reg.registeredAt ? new Date(reg.registeredAt).toLocaleString() : "-"}
                                    </td>
                                    <td className="py-3 px-4">
                                      <span className={`px-2 py-0.5 border-2 border-black text-xs font-bold uppercase ${reg.status === "registered"
                      ...
                      <div className="bg-white border-2 border-black p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                        <h3 className="text-lg font-black uppercase text-black mb-6">Select an event to view registrations</h3>
                        {events.length === 0 ? (
                          <div className="text-center py-12 border-2 border-dashed border-black">
                            ...
                            <p className="text-slate-500 font-bold uppercase mb-4">No events available</p>
                            <button
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {events.map((ev) => (
                              <div key={ev._id} className="bg-white border-2 border-black hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] transition-all group cursor-pointer flex flex-col" onClick={() => loadRegistrations(ev)}>
                                {/* Event Image */}
                                ...
                                <div className="p-4 flex-1 flex flex-col">
                                  <h4 className="font-black text-black uppercase leading-tight line-clamp-2 mb-3 group-hover:underline">{ev.title}</h4>
                                  <div className="space-y-2 mb-4 flex-1">
                                    ...
                                    <div className="flex items-center gap-2 text-xs font-medium text-slate-600">
                                      ...
                                      <span className="line-clamp-1 uppercase">{ev.location || (ev.isOnline ? "Online" : "Location not set")}</span>
                                    </div>
                                    <div className="flex items-center gap-2 text-xs">
                                      ...
                                      {ev.capacity > 0 && (
                                        <span className="text-slate-500 font-mono">/ {ev.capacity}</span>
                                      )}
                                    </div>
                                  </div>
                                  ...
                                  <button
                                    className="w-full px-3 py-2 bg-black text-white hover:bg-neutral-800 border-2 border-black text-xs font-bold uppercase shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-[1px] active:translate-y-[1px] transition-all flex items-center justify-center gap-2"
                                  >
                          ...
                          <div className="flex-1 animate-fadeIn">
                            {aiInsightsLoading ? (
                              <div className="flex flex-col items-center justify-center py-20 text-black space-y-4">
                                ...
                                <p className="font-black uppercase tracking-widest animate-pulse">Generating AI Insights...</p>
                              </div>
                              ...
                              <div className="flex flex-col items-center justify-center py-20 border-2 border-dashed border-black/20 bg-neutral-50">
                                ...
                                <p className="font-black uppercase tracking-widest text-black/40">Not enough reviews yet to generate insights.</p>
                              </div>
                              ...
                              <div className="space-y-8">
                                <div className="bg-yellow-100 border-2 border-black p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                                  ...
                                  <p className="text-black font-medium leading-relaxed">{aiInsights.overallSentiment}</p>
                                </div>
                                  ...
                                  <div className="bg-emerald-50 border-2 border-green-600 p-6 shadow-[4px_4px_0px_0px_rgba(22,163,74,1)] flex flex-col">
                                    ...
                                    <ul className="space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                                      {aiInsights.topPositives?.map((pos, idx) => (
                                        <li key={idx} className="flex gap-3 text-green-900 font-medium text-sm items-start bg-emerald-100/50 p-2 border-l-2 border-green-500 rounded-r">
                                  ...
                                  <div className="bg-red-50 border-2 border-red-600 p-6 shadow-[4px_4px_0px_0px_rgba(220,38,38,1)] flex flex-col">
                                    ...
                                    <ul className="space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                                      {aiInsights.keyAreasForImprovement?.map((area, idx) => (
                                        <li key={idx} className="flex gap-3 text-red-900 font-medium text-sm items-start bg-red-100/50 p-2 border-l-2 border-red-500 rounded-r">
                                ...
                                <div className="bg-white border-2 border-black p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                                  ...
                                  <div className="space-y-4">
                                    {aiInsights.actionableSuggestions?.map((sug, idx) => (
                                      <div key={idx} className="flex items-center gap-4 bg-neutral-50 border-2 border-black p-4 group hover:bg-yellow-50 hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-1 transition-all cursor-default">
                          ...
                          <div className="flex-1">
                            {feedbacks.length === 0 ? (
                              <div className="text-center py-12 border-2 border-dashed border-black/20">
                                ...
                                <p className="text-slate-500 text-lg font-bold uppercase">No feedback yet</p>
                              </div>
                            ) : (
                              <div className="space-y-4">
                                {feedbackStats && feedbackStats.sentimentBreakdown && (
                                  <div className="bg-white border-2 border-black p-4 mb-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                                    <h3 className="font-black text-lg uppercase tracking-widest border-b-2 border-black pb-2 mb-4">AI Sentiment Summary</h3>
                                    <div className="flex flex-wrap gap-4 items-center">
                                      <div className="flex-1 min-w-[120px] bg-emerald-100 border-2 border-black p-4 text-center">
                                        <div className="text-3xl font-black text-emerald-700">{feedbackStats.sentimentBreakdown.Positive || 0}</div>
                                        <div className="text-xs font-bold uppercase text-emerald-900 tracking-widest mt-1">Positive</div>
                                      </div>
                                      <div className="flex-1 min-w-[120px] bg-neutral-100 border-2 border-black p-4 text-center">
                                        <div className="text-3xl font-black text-neutral-700">{feedbackStats.sentimentBreakdown.Neutral || 0}</div>
                                        <div className="text-xs font-bold uppercase text-neutral-900 tracking-widest mt-1">Neutral</div>
                                      </div>
                                      <div className="flex-1 min-w-[120px] bg-red-100 border-2 border-black p-4 text-center">
                                        <div className="text-3xl font-black text-red-700">{feedbackStats.sentimentBreakdown.Negative || 0}</div>
                                        <div className="text-xs font-bold uppercase text-red-900 tracking-widest mt-1">Negative</div>
                                      </div>
                                    </div>
                                  </div>
                                )}
                                {feedbacks.map((feedback, idx) => (
                                  <div key={idx} className="bg-neutral-50 p-6 border-2 border-black hover:bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] transition-all duration-300">
                                    <div className="flex items-start justify-between mb-3">
                                      <div>
                                        <p className="font-black text-black uppercase text-lg">{feedback.isAnonymous ? "Anonymous" : (feedback.reviewerId?.fullname || "Anonymous")}</p>
                                        {!feedback.isAnonymous && (
                                          <p className="text-slate-600 text-sm font-mono">{feedback.reviewerId?.email || ""}</p>
                                        )}
                                      </div>
                                      <div className="flex flex-col items-end gap-2">
                                        <div className="flex items-center space-x-1 bg-black text-white px-3 py-1 border-2 border-black">
                                          <span className="font-bold text-sm tracking-widest">RATING:</span>
                                          <span className="font-black text-lg ml-2">{feedback.overallRating || 0}/5</span>
                                        </div>
                                        {feedback.sentimentLabel && (
                                          <span className={`px-2 py-0.5 text-[10px] font-black uppercase tracking-widest border-2 border-black ${feedback.sentimentLabel === 'Positive' ? 'bg-emerald-400 text-black' :
                                            feedback.sentimentLabel === 'Negative' ? 'bg-red-400 text-white' :
                                    ...
                                    {Array.isArray(feedback.reviewFields) && feedback.reviewFields.length > 0 && (
                                      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t-2 border-dashed border-black/10">
                                        {feedback.reviewFields.map((f, i) => (
                                          <div key={i} className="bg-white p-3 border-2 border-black/10">
                                            <div className="text-xs font-bold uppercase text-slate-500 mb-1">{f.fieldName}</div>
                                            {f.fieldType === "rating" ? (
                                              <div className="flex items-center">
                                                ...
                                                <span className="ml-2 text-black font-bold text-xs">({f.rating || 0}/5)</span>
                                              </div>
                                            ) : (
                                              <div className="text-black font-medium text-sm break-words leading-relaxed">{String(f.value || "").trim() || "-"}</div>
                                            )}
                                          </div>
                                        ))}
                                      </div>
                ...
                {activeTab === "discover" && (
                  <div className="space-y-6 animate-fadeIn">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      <div>
                        <h2 className="text-2xl font-black uppercase text-black">Events from Other Hosts</h2>
                        <p className="text-slate-600 font-medium mt-1">Explore and discover events from other colleges and institutions</p>
                      </div>
                      <button
                    ...
                    {discoverSearch && (
                      <div className="text-sm font-bold text-black uppercase">
                        Found <span className="text-blue-700">{filteredDiscoverEvents.length}</span> event{filteredDiscoverEvents.length !== 1 ? 's' : ''}
                      </div>
                    )}
                      ...
                      {filteredDiscoverEvents.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                          {filteredDiscoverEvents.map((ev) => (
                            <div key={ev._id} className="bg-white border-2 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:shadow-[16px_16px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[-8px] hover:translate-y-[-8px] transition-all duration-300 flex flex-col group overflow-hidden">
                              {/* Event Image */}
                              <div className="relative h-48 bg-neutral-100 border-b-2 border-black overflow-hidden">
                                {ev.imageUrl ? (
                                  <img
                              ...
                              {/* Event Info */}
                              <div className="p-6 flex-1 flex flex-col">
                                <h4 className="font-black text-black text-lg mb-2 line-clamp-2 uppercase leading-tight group-hover:underline decoration-2 underline-offset-2">{ev.title}</h4>
                                <p className="text-sm text-slate-600 mb-6 line-clamp-2 font-medium">
                                ...
                                <div className="space-y-3 mb-6 font-mono text-xs font-medium">
                                  <div className="flex items-center gap-3 text-slate-700">
                                    ...
                                    <span className="uppercase">{new Date(ev.date).toLocaleDateString()}</span>
                                  </div>
                                  <div className="flex items-center gap-3 text-slate-700">
                                    <MapPin className="w-4 h-4 text-black" />
                                    <span className="line-clamp-1 uppercase">{ev.location || "TBA"}</span>
                                  </div>
                                  <div className="flex items-center gap-3 text-slate-700">
                                    <Users className="w-4 h-4 text-black" />
                                ...
                                {Array.isArray(ev.tags) && ev.tags.length > 0 && (
                                  <div className="flex gap-2 flex-wrap mb-6">
                                    {ev.tags.slice(0, 4).map((t, i) => (
                                      <span key={i} className="px-2 py-0.5 border-2 border-black bg-neutral-100 text-black text-xs font-bold uppercase">
                                        {t}
                                      </span>
                                    ))}
                                    {ev.tags.length > 4 && (
                                ...
                                <div className="mt-auto flex items-center justify-between pt-4 border-t-2 border-dashed border-black/20">
                                  <div className="text-xs font-bold uppercase text-slate-600">
                                    <span className="text-black">{ev.registrations?.length || 0}</span> registered
                                  </div>
                                  {ev.price > 0 ? (
                                    <span className="text-xs font-black uppercase text-green-700">{ev.currency} {ev.price}</span>
                                  ) : (
                                    <span className="text-xs font-black uppercase text-green-700">Free</span>
                                  )}
                                </div>
                              </div>
                            </div>
                ...
                {activeTab === "analytics" && (
                  <div className="space-y-6 animate-fadeIn">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <input type="date" value={dateStart} onChange={e => setDateStart(e.target.value)} className="px-3 py-2 bg-white border-2 border-black text-sm font-bold uppercase outline-none shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] focus:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[-1px] hover:translate-y-[-1px] transition-all" />
                        <span className="text-black font-black">TO</span>
                        <input type="date" value={dateEnd} onChange={e => setDateEnd(e.target.value)} className="px-3 py-2 bg-white border-2 border-black text-sm font-bold uppercase outline-none shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] focus:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[-1px] hover:translate-y-[-1px] transition-all" />
                      </div>
                    </div>
                          ...
                          <>
                            <div className="space-y-0 mb-6 border-2 border-black">
                              {[
                                ...
                                <div key={label} className={`flex justify-between items-center p-3 ${idx !== 4 ? 'border-b-2 border-black' : ''} hover:bg-neutral-50`}>
                            ...
                            <div className="h-64 border-2 border-black p-2 bg-neutral-50">
                              <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={registrationsSeries} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                                  ...
                                  <XAxis dataKey="date" tick={{ fontSize: 10, fontWeight: 'bold' }} stroke="#000" />
                                  <YAxis allowDecimals={false} tick={{ fontSize: 10, fontWeight: 'bold' }} stroke="#000" />
                                  <Tooltip
                                  ...
                                  <Line type="step" dataKey="count" stroke="#000" strokeWidth={3} dot={{ r: 4, strokeWidth: 2, fill: "white", stroke: "black" }} activeDot={{ r: 6, strokeWidth: 2, fill: "black" }} name="Registrations" />
                                </LineChart>
                              </ResponsiveContainer>
                            </div>
                          </>
                          ...
                          <div className="h-80 border-2 border-black p-2 bg-neutral-50">
                            <ResponsiveContainer width="100%" height="100%">
                              <BarChart data={registrationsByEvent} margin={{ top: 10, right: 20, left: 0, bottom: 40 }}>
                                <CartesianGrid strokeDasharray="3 3" stroke="#000" strokeOpacity={0.1} />
                                <XAxis dataKey="name" tick={{ fontSize: 10, fontWeight: 'bold' }} interval={0} angle={-25} textAnchor="end" height={60} stroke="#000" />
                                <YAxis allowDecimals={false} tick={{ fontSize: 10, fontWeight: 'bold' }} stroke="#000" />
                                <Tooltip contentStyle={{ border: '2px solid black', borderRadius: '0px', boxShadow: '4px 4px 0px 0px rgba(0,0,0,1)' }} />
                                <Legend wrapperStyle={{ paddingTop: '10px', fontWeight: 'bold' }} />
                                <Bar dataKey="value" fill="#000" name="Registrations" radius={[0, 0, 0, 0]} />
                              </BarChart>
                            </ResponsiveContainer>
                          </div>
                        )}
                          ...
                          <div className="h-64 border-2 border-black p-2 bg-neutral-50">
                            <ResponsiveContainer width="100%" height="100%">
                              <LineChart data={eventsByDay} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                                ...
                                <Tooltip contentStyle={{ border: '2px solid black', borderRadius: '0px', boxShadow: '4px 4px 0px 0px rgba(0,0,0,1)' }} />
                                <Legend wrapperStyle={{ paddingTop: '10px', fontWeight: 'bold' }} />
                                <Line type="step" dataKey="count" stroke="#000" strokeWidth={3} dot={{ r: 4, strokeWidth: 2, fill: "white", stroke: "black" }} name="Events" />
                              </LineChart>
                            </ResponsiveContainer>
                          </div>
                        )}
                      ...
                      <div className="bg-white border-2 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                        ...
                        <div className="space-y-3 max-h-80 overflow-y-auto mb-6 pr-2 custom-scrollbar">
                          {notificationsOverRange.slice(0, 5).map((notification, idx) => (
                            <div key={idx} className="flex items-start space-x-3 p-3 bg-white border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-transform hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none">
                              <BellRing className="w-4 h-4 text-black shrink-0 mt-1" />
                              <div className="flex-1">
                                <p className="text-sm text-black font-bold uppercase leading-tight">{notification.message}</p>
                        ...
                        <div className="h-64 mt-4 border-t-2 border-black pt-6">
                          <h4 className="text-xs font-black uppercase text-black mb-4">Events Status Distribution</h4>
                          <div className="h-full border-2 border-black p-2 bg-neutral-50">
                            <ResponsiveContainer width="100%" height="100%">
                              <BarChart data={eventsByStatus} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                                <CartesianGrid strokeDasharray="3 3" stroke="#000" strokeOpacity={0.1} />
                                <XAxis dataKey="name" tick={{ fontSize: 10, fontWeight: 'bold' }} stroke="#000" />
                                <YAxis allowDecimals={false} tick={{ fontSize: 10, fontWeight: 'bold' }} stroke="#000" />
                                <Tooltip contentStyle={{ border: '2px solid black', borderRadius: '0px', boxShadow: '4px 4px 0px 0px rgba(0,0,0,1)' }} />
                                <Legend wrapperStyle={{ paddingTop: '10px', fontWeight: 'bold' }} />
                ...
                <div className="bg-white border-2 border-black p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] animate-scaleIn">
                  ...
                  <div className="space-y-4">
                    {reviewFields.map((f, idx) => (
                      <div key={idx} className="bg-neutral-50 p-4 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                          <div className="md:col-span-2">
                            <label className="block text-xs font-bold uppercase text-black mb-1">Field Name</label>
                            <input
                              type="text"
                              value={f.fieldName}
                          ...
                          <div>
                            <label className="block text-xs font-bold uppercase text-black mb-1">Field Type</label>
                            <select
                              value={f.fieldType}
                        ...
                        {f.fieldType !== "rating" && (
                          <div className="mt-3">
                            <label className="block text-xs font-bold uppercase text-black mb-1">Placeholder</label>
                            <input
                              type="text"
                              value={f.placeholder || ""}
                        ...
                        <div className="mt-3 flex justify-between items-center border-t-2 border-dashed border-black/20 pt-2">
                          <span className="text-xs font-mono font-bold text-slate-500">ORDER: {idx + 1}</span>
                          <button
                  ...
                  <form onSubmit={submitForm} className="space-y-8">
                    ...
                    <div className="bg-neutral-50 border-2 border-black p-6">
                      <h3 className="text-xl font-black uppercase mb-6 text-black flex items-center gap-2">
                        <span className="bg-black text-white px-2 py-1 text-sm">01</span> Basic Information
                      </h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          <label className="block text-xs font-bold uppercase text-black mb-2">Event Title *</label>
                          <input
                            type="text"
                            value={form.title}
                            ...
                            required
                          />
                          {formErrors.title && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.title}</p>}
                        </div>
                        <div>
                          <label className="block text-xs font-bold uppercase text-black mb-2">Category</label>
                          <div className="relative">
                            <select
                              value={form.category}
                      ...
                      <div className="mt-6">
                        <label className="block text-xs font-bold uppercase text-black mb-2">Short Description (Max 150 chars) *</label>
                        <input
                          type="text"
                        ...
                        <div className="flex justify-between text-xs font-bold text-slate-500 mt-1 uppercase">
                          <span>{formErrors.shortDescription && <span className="text-red-600">{formErrors.shortDescription}</span>}</span>
                          <span>{form.shortDescription.length}/150</span>
                        </div>
                      </div>
                      <div className="mt-6">
                        <label className="block text-xs font-bold uppercase text-black mb-2">Full Description *</label>
                        <textarea
                          ...
                          required
                        />
                        {formErrors.description && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.description}</p>}
                      </div>
                    </div>
                    ...
                    <div className="bg-neutral-50 border-2 border-black p-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          <label className="block text-xs font-bold uppercase text-black mb-2">Start Date & Time *</label>
                          <input
                            type="datetime-local"
                            value={form.date}
                            ...
                            required
                          />
                          {formErrors.date && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.date}</p>}
                        </div>
                        <div>
                          <label className="block text-xs font-bold uppercase text-black mb-2">End Date & Time</label>
                          <input
                            type="datetime-local"
                            value={form.endDate}
                          ...
                          />
                          {formErrors.endDate && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.endDate}</p>}
                        </div>
                        <div>
                          <label className="block text-xs font-bold uppercase text-black mb-2">Registration Deadline *</label>
                          <input
                            type="datetime-local"
                            ...
                            required
                          />
                          {formErrors.registrationDeadline && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.registrationDeadline}</p>}
                        </div>
                      </div>
                      {sameDayEvents.length > 0 && (
                        <div className="mt-6 p-4 bg-yellow-100 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                          ...
                          <ul className="list-disc list-inside text-black text-xs font-bold space-y-1 max-h-40 overflow-auto font-mono">
                            {sameDayEvents.map((e) => (
                              <li key={e._id}>
                                <span className="uppercase">{e.title}</span> • {new Date(e.date).toLocaleTimeString()} • {e.location || (e.isOnline ? "Online" : "")}
                              </li>
                            ))}
                          </ul>
                        </div>
                    ...
                    <div className="bg-neutral-50 border-2 border-black p-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          <label className="block text-xs font-bold uppercase text-black mb-2">Event Type</label>
                          <div className="flex space-x-6">
                            <label className="flex items-center cursor-pointer group">
                              ...
                              />
                              <span className="text-black font-bold uppercase group-hover:underline">In-Person</span>
                            </label>
                            <label className="flex items-center cursor-pointer group">
                              <input
                                type="radio"
                                ...
                                className="mr-3 w-5 h-5 border-2 border-black text-black focus:ring-0 cursor-pointer"
                              />
                              <span className="text-black font-bold uppercase group-hover:underline">Online</span>
                            </label>
                          </div>
                        </div>
                        <div>
                          <label className="block text-xs font-bold uppercase text-black mb-2">Location/Venue *</label>
                          <input
                            type="text"
                            value={form.location}
                            ...
                            required
                          />
                          {formErrors.location && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.location}</p>}
                        </div>
                      </div>
                      {form.isOnline && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                          <div>
                            <label className="block text-xs font-bold uppercase text-black mb-2">Platform</label>
                            <div className="relative">
                              <select
                                value={form.platform}
                          ...
                          <div>
                            <label className="block text-xs font-bold uppercase text-black mb-2">Meeting Link</label>
                            <input
                              type="url"
                            ...
                            />
                            {formErrors.meetingLink && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.meetingLink}</p>}
                          </div>
                        </div>
                      )}
                      {!form.isOnline && (
                        <>
                          <div className="mt-6">
                            <label className="block text-xs font-bold uppercase text-black mb-2">Address *</label>
                            <input
                              type="text"
                              value={form.address}
                              ...
                              required
                            />
                            {formErrors.address && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.address}</p>}
                          </div>
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                            <div>
                              <label className="block text-xs font-bold uppercase text-black mb-2">City *</label>
                              <input
                                type="text"
                                value={form.city}
                                ...
                                required
                              />
                              {formErrors.city && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.city}</p>}
                            </div>
                            <div>
                              <label className="block text-xs font-bold uppercase text-black mb-2">State *</label>
                              <input
                                type="text"
                                value={form.state}
                                ...
                                required
                              />
                              {formErrors.state && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.state}</p>}
                            </div>
                            <div>
                              <label className="block text-xs font-bold uppercase text-black mb-2">Pincode</label>
                              <input
                                type="text"
                                value={form.pincode}
                                ...
                                maxLength="6"
                              />
                              {formErrors.pincode && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.pincode}</p>}
                            </div>
                          </div>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                            <div>
                              <label className="block text-xs font-bold uppercase text-black mb-2">Latitude</label>
                              <input
                                type="number"
                                step="any"
                                value={form.latitude}
                              ...
                              />
                              {formErrors.latitude && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.latitude}</p>}
                            </div>
                            <div>
                              <label className="block text-xs font-bold uppercase text-black mb-2">Longitude</label>
                              <input
                                type="number"
                                step="any"
                              ...
                              />
                              {formErrors.longitude && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.longitude}</p>}
                            </div>
                          </div>
                    ...
                    <div className="bg-neutral-50 border-2 border-black p-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                          <label className="block text-xs font-bold uppercase text-black mb-2">Capacity *</label>
                          <input
                            type="text"
                            value={form.capacity}
                            ...
                            required
                          />
                          {formErrors.capacity && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.capacity}</p>}
                        </div>
                        <div>
                          <label className="block text-xs font-bold uppercase text-black mb-2">Price</label>
                          <div className="flex">
                            <input
                              type="text"
                              value={form.price}
                          ...
                          {formErrors.price && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.price}</p>}
                        </div>
                        <div className="md:col-span-2">
                          <label className="block text-xs font-bold uppercase text-black mb-2">Tags</label>
                          <input
                            type="text"
                            value={form.tags}
                          ...
                          />
                          <div className="text-xs font-bold text-slate-500 mt-1 uppercase">Up to 10 tags. Comma-separated.</div>
                          {formErrors.tags && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.tags}</p>}
                        </div>
                      </div>
                      <div className="mt-6">
                        <label className="block text-xs font-bold uppercase text-black mb-2">Requirements</label>
                        <textarea
                      ...
                      <div className="mt-6">
                        <label className="block text-xs font-bold uppercase text-black mb-2">Agenda</label>
                        <textarea
                          value={form.agenda}
                    ...
                    <div className="bg-neutral-50 border-2 border-black p-6">
                      <h3 className="text-xl font-black uppercase mb-6 text-black flex items-center gap-2">
                        <span className="bg-black text-white px-2 py-1 text-sm">05</span> Contact Information
                      </h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          <label className="block text-xs font-bold uppercase text-black mb-2">Contact Email *</label>
                          <input
                            type="email"
                            ...
                            required
                          />
                          {formErrors.contactEmail && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.contactEmail}</p>}
                        </div>
                        <div>
                          <label className="block text-xs font-bold uppercase text-black mb-2">Contact Phone *</label>
                          ...
                          {formErrors.contactPhone && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.contactPhone}</p>}
                        </div>
                      </div>
                      <div className="mt-6">
                        <label className="block text-xs font-bold uppercase text-black mb-2">Website</label>
                        <input
                          ...
                          placeholder="https://example.com"
                        />
                        {formErrors.website && <p className="text-red-600 font-bold text-xs mt-1 uppercase">{formErrors.website}</p>}
                      </div>
                      <div className="mt-6">
                        <label className="block text-xs font-bold uppercase text-black mb-2">Event Image URL</label>
                        <input
                          type="url"
                          value={form.imageUrl}
