...
const FAQ = [
  {
    k: /register.*event|how .*register/i,
    a: 'To register: open the event, click Register. If it is completed or past, registration is disabled.',
  },
  {
    k: /completed|past.*register/i,
    a: 'Events marked completed or with a past date cannot be registered. Try future dates.',
  },
  {
    k: /certificate|download/i,
    a: 'Your certificates are under Profile → Certificates. You can also verify via the certificate link.',
  },
  {
    k: /verify.*(email|phone)|otp/i,
    a: 'Verify email/phone in Profile → Credentials (OTP). If SMS fails in dev, a dev OTP may be returned.',
  },
  {
    k: /calendar/i,
    a: 'Use the Calendar button to open the overlay, Month/Week switch, and click a day chip to view details.',
  },
  {
    k: /bookmarks?/i,
    a: 'Bookmark events using the bookmark icon; view them in the Bookmarks tab.',
  },
];
    ...
    if (usingAI && q) {
      try {
        ...
        const upcomingEvents = await Event.find({
          ...
          .sort({ date: 1 })
          .limit(5)
          .select('title date category location')
          .lean();

        const eventContext =
          upcomingEvents.length > 0
            ? `\nHere are some upcoming events you can recommend: ${upcomingEvents.map((e) => `${e.title} (${new Date(e.date).toLocaleDateString()}, ${e.category} at ${e.location})`).join('; ')}.`
            : '';
