...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    const users = await User.find({ _id: { $in: friendIds }, role: 'student' })
      .select('_id fullname username email institute interests profilePic level role')
      .lean();
    res.json(users);
  } catch (e) {
...
router.get('/search', authenticateToken, async (req, res) => {
  try {
    ...
    const users = await User.find({
      ...
      .select('_id fullname username email institute profilePic role')
      .limit(10)
      .lean();
    res.json(users);
  } catch (e) {
...
router.get('/requests', authenticateToken, async (req, res) => {
  try {
    const inboundRaw = await FriendRequest.find({ to: req.user.id, status: 'pending' })
      .populate('from', '_id fullname username institute profilePic level role')
      .lean();
    const outboundRaw = await FriendRequest.find({ from: req.user.id, status: 'pending' })
      .populate('to', '_id fullname username institute profilePic level role')
      .lean();
...
router.post('/requests', authenticateToken, async (req, res) => {
  try {
    const { to } = req.body || {};
    if (!to || String(to) === String(req.user.id))
...
router.put('/requests/:id/accept', authenticateToken, async (req, res) => {
  try {
    ...
    // Gamification: Award points to both users for making a connection
...
router.get('/suggestions', authenticateToken, async (req, res) => {
  try {
    ...
    const candidates = await User.find({ _id: { $ne: req.user.id }, role: 'student' })
      .select('_id fullname username institute interests profilePic level role')
      .lean();
    ...
    accepted.forEach((e) => {
      addEdge(String(e.from), String(e.to));
      addEdge(String(e.to), String(e.from));
    });
    ...
    const myEvents = await Event.find({
      'registrations.studentId': req.user.id,
      isDeleted: { $ne: true },
    })
      .select('registrations')
      .lean();
    // Build per-event attendee sets for quick lookup
    const eventAttendeeSets = myEvents.map(
      (ev) =>
        new Set(
          (ev.registrations || [])
    ...
    const W1 = 3,
      W2 = 2,
      W3 = 1,
      W4 = 3; // sameInstitute, mutualFriends, sharedInterests, mutualEvents
    const CAP = 10;
    ...
    const ranked = candidates
      ...
      .map((u) => {
        ...
        const mutuals = [...(graph.get(String(u._id)) || new Set())].filter((x) =>
          mySet.has(x),
        ).length;
        const sharedInterests = (u.interests || []).reduce(
          (acc, it) => acc + (myInterests.has(String(it).toLowerCase()) ? 1 : 0),
          0,
        );
        ...
        if (eventAttendeeSets.length) {
          ...
          for (const set of eventAttendeeSets) {
            if (set.has(uid)) mutualEvents++;
          }
        }
        const score =
          sameInstitute * W1 +
          Math.min(CAP, mutuals) * W2 +
          sharedInterests * W3 +
          Math.min(CAP, mutualEvents) * W4;
        return {
          ...u,
