    ...
    if (isEmailLike && q) {
      ...
      or.push({
        $and: [
          ...
          { 'settings.privacy.searchableByEmail': { $ne: false } },
        ],
      });
    }
    const filter = q ? { $or: or } : {};
    const users = await User.find(filter)
      .select('_id fullname username email institute interests profilePic level settings.privacy')
      .skip((page - 1) * limit)
      .limit(limit)
      .lean();
...
router.get('/:id/mutual-events', authenticateToken, async (req, res) => {
  try {
    ...
    const myEvents = await Event.find({
      'registrations.studentId': other,
      isDeleted: { $ne: true },
    })
      .select('registrations date')
      .lean();
    const counts = Object.fromEntries(
      candidates.map((id) => [String(id), { total: 0, recent: 0 }]),
    );
    const recentDays = Math.max(0, parseInt(req.query.recent || '0', 10));
    const recentCutoff =
      recentDays > 0 ? new Date(Date.now() - recentDays * 24 * 60 * 60 * 1000) : null;
    for (const ev of myEvents) {
      ...
      for (const id of candidates) {
        if (set.has(String(id))) {
          ...
          if (recentCutoff && ev.date && new Date(ev.date) >= recentCutoff)
            counts[String(id)].recent = (counts[String(id)].recent || 0) + 1;
        }
      }
    }
    res.json(counts);
...
router.get('/:id/profile', authenticateToken, async (req, res) => {
  try {
    ...
    const user = await User.findById(other)
      .select(
        '_id fullname username email institute course city pincode age countryCode interests profilePic level bannerUrl bio displayBadges role settings',
      )
      .lean();
    ...
    const resp = {
      ...
      bannerUrl:
        privacy.onlyFriendsCanViewProfile && !isFriend && !isSelf ? undefined : user.bannerUrl,
      institute:
        privacy.onlyFriendsCanViewProfile && !isFriend && !isSelf ? undefined : user.institute,
      ...
      countryCode:
        privacy.onlyFriendsCanViewProfile && !isFriend && !isSelf ? undefined : user.countryCode,
      email: privacy.onlyFriendsCanViewProfile && !isFriend && !isSelf ? undefined : user.email,
      interests:
        privacy.onlyFriendsCanViewProfile && !isFriend && !isSelf ? [] : user.interests || [],
      bio: privacy.onlyFriendsCanViewProfile && !isFriend && !isSelf ? undefined : user.bio,
      role: user.role,
      level: user.level || 1,
      displayBadges:
        privacy.showBadgesPublic === false && !isSelf && !isFriend ? [] : user.displayBadges || [],
    };
    res.json(resp);
  } catch (e) {
...
router.get('/:id/mutuals', authenticateToken, async (req, res) => {
  try {
    ...
    const edges = await FriendRequest.find({
      status: 'accepted',
      $or: [{ from: me }, { to: me }, { from: other }, { to: other }],
    })
      .select('from to')
      .lean();
    const setMe = new Set();
    ...
    edges.forEach((e) => {
      const a = String(e.from),
        b = String(e.to);
      if (a === me) setMe.add(b);
      else if (b === me) setMe.add(a);
      if (a === other) setOther.add(b);
      else if (b === other) setOther.add(a);
    });
...
router.get('/:id/stats', authenticateToken, async (req, res) => {
  try {
    ...
    const byCategory = regs.reduce((acc, r) => {
    ...
    if (dates.length) {
      ...
      for (let i = 0; i < dates.length - 1; i++) {
        const diffDays = Math.round(
          (dates[i].setHours(0, 0, 0, 0) - dates[i + 1].setHours(0, 0, 0, 0)) /
          (1000 * 60 * 60 * 24),
        );
...
router.get('/:id/certificates', authenticateToken, async (req, res) => {
  try {
    ...
    const certs = await Certificate.find({ studentId: other })
      .select('_id title url createdAt')
      .sort({ createdAt: -1 })
      .lean();
    res.json(
      certs.map((c) => ({ id: String(c._id), title: c.title, url: c.url, issuedAt: c.createdAt })),
    );
  } catch (e) {
