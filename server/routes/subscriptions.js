...
router.get('/', authenticateToken, async (req, res) => {
  try {
    const user = await User.findById(req.user.id)
      .populate('subscribedHosts', 'fullname email username institute city profilePic')
      .lean();
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    // Award points
    await gamificationController.awardPoints(req.user.id, 'SUBSCRIBE_HOST');
...
router.get('/events', authenticateToken, async (req, res) => {
  try {
    ...
    const recentEvents = await Event.find({
      ...
      .populate('hostId', 'fullname email username institute city profilePic')
      .sort({ createdAt: -1 })
      .limit(20);
